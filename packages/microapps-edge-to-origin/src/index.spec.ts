...
describe('handler', () => {
  ...
  describe('host header', () => {
    const testCases = [
      { replaceHostHeader: true, expectedHost: 'test-origin.pwrdrvr.com' },
      { replaceHostHeader: false, expectedHost: 'test.pwrdrvr.com' },
    ];

    it.each(testCases)(
      'host header when replaceHostHeader is %p',
      async ({ replaceHostHeader, expectedHost }) => {
        theConfig.replaceHostHeader = replaceHostHeader;
        ...
        expect(signedRequest!.headers!['host'][0].value).toEqual(expectedHost);
      },
    );
  });
  ...
  describe('x-forwarded-host header', () => {
    const testCases = [
      { addXForwardedHostHeader: true, expectedHost: 'test.pwrdrvr.com' },
      { addXForwardedHostHeader: false, expectedHost: undefined },
    ];

    it.each(testCases)(
      'host header when replaceHostHeader is %p',
      async ({ addXForwardedHostHeader, expectedHost }) => {
        theConfig.addXForwardedHostHeader = addXForwardedHostHeader;
        ...
        expect(signedRequest).toBeDefined();
        expect(signedRequest!.headers).toBeDefined();
        if (addXForwardedHostHeader) {
          expect(signedRequest!.headers!['x-forwarded-host'][0].value).toEqual(expectedHost);
        } else {
